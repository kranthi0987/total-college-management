...
//        layoutOfPopup.addView(toText);
    ...
    private void setTheTasks(DatePicker from, DatePicker to, int tasks) {
        ...
        if (from != null)
            ((TextView) findViewById(R.id.fromRange)).setText(getFormattedDate(from).replaceAll("%20", "/"));
        if (to != null)
            ((TextView) findViewById(R.id.toRange)).setText(getFormattedDate(to).replaceAll("%20", "/"));
...
//                Toast.makeText(InvigilationDetails.this, df.format(d), Toast.LENGTH_SHORT).show();
    ...
    private class ColorDecorator implements DayDecorator {
        ...
        public void decorate(DayView dayView) {
            ...
            if (NetworkUtils.isToday(cal))
                dayView.setBackgroundColor(getResources().getColor(R.color.calenderCurrentDay));
            if (data != null && data.containsKey(ds))
